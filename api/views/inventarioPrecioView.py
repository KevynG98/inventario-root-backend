...
from ..serializers.inventarioPrecioSerializer import PrecioInventarioSerializer


@swagger_auto_schema(method='get', operation_summary="Listar precios de inventario", tags=["inventario-precio"])
@api_view(['GET'])
def listar_precios(request):
    """
    Lista todos los precios activos de inventario.
    """
    precios = PrecioInventario.objects.filter(is_active=True)
    serializer = PrecioInventarioSerializer(precios, many=True)
    return Response(serializer.data)


@swagger_auto_schema(method='post', request_body=PrecioInventarioSerializer, operation_summary="Crear nuevo precio", tags=["inventario-precio"])
@api_view(['POST'])
def crear_precio(request):
    """
    Crea un nuevo registro de precio para un producto del inventario.
    """
    print(request.data)
    serializer = PrecioInventarioSerializer(data=request.data)
...
@swagger_auto_schema(method='put', request_body=PrecioInventarioSerializer, operation_summary="Actualizar precio de inventario", tags=["inventario-precio"])
@api_view(['PUT'])
def actualizar_precio(request, pk):
    """
    Actualiza un precio de inventario por ID.
    """
    try:
        precio = PrecioInventario.objects.get(pk=pk, is_active=True)
    except PrecioInventario.DoesNotExist:
        return Response({"error": "Precio no encontrado"}, status=status.HTTP_404_NOT_FOUND)

    print("🔍 Datos recibidos:", request.data)
    serializer = PrecioInventarioSerializer(precio, data=request.data)
    if serializer.is_valid():
        serializer.save()
        return Response(serializer.data)
...
def eliminar_precio(request, pk):
    """
    Elimina (soft delete) un precio de inventario por ID.
    """
    try:
        precio = PrecioInventario.objects.get(pk=pk, is_active=True)
...
# -----------------------------
def _detect_inventario_fields():
    """
    Detecta en tiempo de ejecución cómo se llaman en InventarioProducto:
    - el código del inventario (p.ej. 'codigo_inventario', 'codigo', 'clave', etc.)
    ...
    """
    inventario_fk = PrecioInventario._meta.get_field('inventario')  # si tu FK NO se llama 'inventario', ajusta aquí
    inventario_model = inventario_fk.remote_field.model
    field_names = {f.name for f in inventario_model._meta.fields}

    candidates_code = ['codigo_inventario', 'codigo', 'clave', 'codigo_item']
...
@swagger_auto_schema(
    ...
    manual_parameters=[
        ...
        openapi.Parameter('inventario_id', openapi.IN_QUERY, description='ID del inventario (exacto)', type=openapi.TYPE_INTEGER),
        openapi.Parameter('codigo_inventario', openapi.IN_QUERY, description='Código del inventario (icontains)', type=openapi.TYPE_STRING),
        ...
        openapi.Parameter('q', openapi.IN_QUERY, description='Búsqueda libre (seguro + campos detectados del inventario)', type=openapi.TYPE_STRING),
...
def buscar_precios(request):
    ...
    - No revienta si el modelo InventarioProducto no tiene 'codigo_barras' u otros campos: detecta primero.
    """
    qs = PrecioInventario.objects.select_related('inventario').all()
    ...
    # Detectar campos reales del modelo InventarioProducto
    code_field, barcode_field, text_field = _detect_inventario_fields()
    # print("Inventario fields:", code_field, barcode_field, text_field)  # útil para debug local

    # Filtros específicos
    inventario_id = request.query_params.get('inventario_id')
    if inventario_id:
        qs = qs.filter(inventario_id=inventario_id)

    codigo_inventario = request.query_params.get('codigo_inventario')
    if codigo_inventario and code_field:
        qs = qs.filter(**{f"inventario__{code_field}__icontains": codigo_inventario})

    codigo_barras = request.query_params.get('codigo_barras')
    if codigo_barras and barcode_field:
        qs = qs.filter(**{f"inventario__{barcode_field}__icontains": codigo_barras})
    ...
    if q:
        ...
        if code_field:
            condiciones.append(Q(**{f"inventario__{code_field}__icontains": q}))
        if barcode_field:
            condiciones.append(Q(**{f"inventario__{barcode_field}__icontains": q}))
        if text_field:
            condiciones.append(Q(**{f"inventario__{text_field}__icontains": q}))
    ...
    serializer = PrecioInventarioSerializer(qs, many=True)
